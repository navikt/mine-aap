...
import { Ettersendelse, InnsendingBackendState, Kvittering, VedleggType } from 'lib/types/types';

const handler = beskyttetApi(async (req, res) => {
  const { ettersendteVedlegg, søknadId }: Ettersendelse = JSON.parse(req.body);

  const ettersending = ettersendteVedlegg[0];
  const kvittering = genererKvittering(ettersending.ettersending.length);
...
const genererKvittering = (antallEttersendelser: number) => {
  return {
    mottattdato: new Date(),
    temaer: {
      vedlegg: {
        type: 'TEMA',
        overskrift: 'Ettersending',
        underblokker: [
          {
            type: 'FELT',
            felt: 'Antall ettersendte dokumenter: ' + antallEttersendelser,
          },
        ],
      },
    },
